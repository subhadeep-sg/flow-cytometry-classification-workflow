...
"""
Using the dataset directory and a PyTorch convolutional neural network
for first phase of classifying the images and accurately obtain the
multiple cell cluster images for further clustering.
"""

st = time.time()
set_seed(42)
# img_directory = '../dataset/prepared'
img_directory = '../dataset/prepared_revised'

df = pd.read_csv('../prepare_data_source/clust_non_clust.csv', index_col=False)

label_list = df['class_label'].tolist()
label_mapping = {'particles': 0, 'single': 1, 'multi': 2}
...
# disp_train_dataset(img_directory)
mean, std = mean_std_computation(img_dir=img_directory)
...
# Loading dataset
train_ds = ImageDataset(img_dir=img_directory, mode='train', transforms=transform)
val_ds = ImageDataset(img_dir=img_directory, mode='val', transforms=test_transform)
...
def train(cnn, epochs, class_weights, train_loader, valid_loader, optimizer, lr_scheduler, loss_fn,
    ...
    for epoch in range(epochs):
        ...
        if val_f1_2 > highest_f1:
            highest_acc = val_weighted_accuracy.item()  # validation_accuracy
            highest_f1 = val_f1_2
            conf_matrix = confusion_matrix(val_labels, val_pred, labels=[0, 1, 2])
            results['f1'] = val_f1
            results['accuracy'] = validation_accuracy
            results['precision'] = val_precision
            results['recall'] = val_recall
            results['roc-auc'] = val_roc_auc
            results['loss'] = val_running_loss
            if save:
        ...
        if verbose:
            ...
            # print(f"Training: Weighted Accuracy: {weighted_accuracy:.4f}, F1 Score('Multi'): {f1_2:.4f}, "
            #       f"F1 Score('Single'): {f1_1:.4f}")
            print(f"Validation: Weighted Accuracy: {val_weighted_accuracy:.4f}, F1 Score('Multi'): {val_f1_2:.4f}, "
                  f"F1 Score('Single'): {val_f1_1:.4f}")
...
# class_weights = [0.2, 0.2, 0.6]  # ['particles', 'single', 'multi']
class_weights = [0.2, 0.3, 0.5]  #[0.2, 0.3, 0.5]
class_weights = torch.FloatTensor(class_weights).cuda()
loss_fn = nn.CrossEntropyLoss(weight=class_weights)
device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
# net = ConvNet(num_channels=3, device=device, batch_size=batch_size).to(device)
#
# lr_factor = 0.9#0.9
# opt1 = optim.SGD(net.parameters(), lr=0.001 * lr_factor, momentum=0.9)
# opt2 = optim.Adam(net.parameters(), lr=0.001 * lr_factor, weight_decay=1e-4)
# opt3 = optim.RMSprop(net.parameters(), lr=0.001 * lr_factor, momentum=0.9)
# opt4 = optim.AdamW(net.parameters(), lr=0.001 * lr_factor, weight_decay=1e-4)
# # lr_scheduler = torch.optim.lr_scheduler.StepLR(opt2, step_size=30, gamma=0.9)
# set_high_acc = 84.0
# set_high_f1 = 0.85
#
# # Training
# model, results, history = train(cnn=net, epochs=100,
#                                 optimizer=opt2, loss_fn=loss_fn,
#                                 class_weights=class_weights,
#                                 lr_scheduler=None,
...
#                                 device=device,
#                                 set_highest_acc=set_high_acc,
#                                 set_highest_f1=set_high_f1,
#                                 verbose=True,
#                                 save=True)
#
...
for fold, (train_index, val_index) in enumerate(kf.split(X_train_val, y_train_val)):
    ...
    net = ConvNet(num_channels=3, img_size=224, batch_size=batch_size).to(device)
...
cv_df.to_csv('cross_validation_p1_results.csv', index=False)
print('Cross validation results saved to cross_validation_p1_results.csv')
